...
## 3. MVP B: 상용 Text-to-Motion Preview/Bake Pipeline

### 3.1 개념

상용 text-to-motion 서비스 또는 manual provider가 생성한 새로운 motion result를 Unreal Editor 안에서 import, retarget, preview, bake한다.

```text
Prompt
-> prompt/constraint 정규화
-> commercial text-to-motion provider
-> generated FBX/BVH/GLB/JSON
-> Unreal import
-> retarget
-> preview
-> approve
...
이 방식은 비용과 라이선스 리스크를 줄이면서도 전체 workflow를 검증할 수 있다.
...
```text
LLM -> procedural JSON -> Control Rig wave
```

Pipeline B:

```text
LLM prompt normalization + procedural JSON constraints
-> commercial text-to-motion result
-> import/retarget/preview
```
...
```text
단순 손흔들기:
Procedural JSON이 비용/속도/안정성에서 유리할 가능성 높음.

복잡한 전신 동작:
Commercial T2M이 자연스러움에서 유리할 수 있으나 retarget/foot/root 문제가 생길 수 있음.
```
